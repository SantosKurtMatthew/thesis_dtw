...
Presented below is Chan's divide-and-conquer algorithm for the bichromatic dominating pairs problem, a prerequisite for Gold and Sharir's algorithm. \cite{ChanDominance}. This algorithm, denoted $\text{dom}(R,B,d')$ returns all pairs of points in $\mathbb{R}^d$, one point $\vec{r}=(r_1,r_2,\dotsc,r_d)$ from a set $R$ and one point $\vec{b}=(b_1,b_2,\dotsc,b_d)$ from a set $B$, such that for all $i\in\{1,2,\dotsc,d\}, r_i\le b_i$, in $O(c_\epsilon^dn^{1+\epsilon}+K)$ time.
...
First, if $n=1$, then no pair of points can be formed, and thus $\text{dom}(L,R,d')=\{\}$ is returned. Second, if $d'=0$, then all pairs of points are returned, i.e., $\text{dom}(L,R,0)=R\times B$.

The recursive case is done when $n>1$ and $d'>0$. Let $m$ be the median of the $d'$-th coordinates of all points in $L$. The following sets are formed:
\begin{align*}
...
\end{align*}
Recursive calls are then made on $(R_l,B_l,d')$, $(R_r,B_r,d')$, and $(R_l,B_r,d'-1)$. (Note that for any $\vec{r}\in R_l$ and $\vec{b}\in B_r$, $r_{d'}<b_{d'}$. Thus, the condition is satisfied for dimension $d'$, and so dom is called on $d'-1$.)
For the combine step, the following is returned:
