...
\includegraphics[width=0.85\linewidth]{Figures/subsets.png}\newline
\textit{Figure 2.1: Subsets Example}
...
\includegraphics[width=0.85\linewidth]{Figures/squares.png}\newline
\textit{Figure 2.2: A box made using the subsets $A_{2}$ and $B_{3}$.}
...
\includegraphics[width=0.85\linewidth]{Figures/staircasepath.png}\newline
\textit{Figure 2.3: A Sample Staircase Path.}
...
\includegraphics[width=0.85\linewidth]{Figures/L_R.png}\newline
\textit{Figure 2.4: A visualization of L and R in a $4\times4$ box.}
...
\includegraphics[width=0.85\linewidth]{Figures/pathVW.png}\newline
\textit{Figure 2.5: Two Sample Paths $P_{v,w}$ and $P^*_{v,w}$.}

Given two paths, $P_{v,w}$ and $P'_{v,w}$, with the same start and end points, these paths will be compared using Chan's Algorithm. The comparison is performed such that one side depends only on points from $A_i$ and the other side on points from only $B_j$. Some path $P_{v,w}$ is chosen as a candidate path for the shortest path; this is repeated for all boxes $D_{i,j}$. This process will have less than $(3^{2g})^{4g^2}$ or $3^{8g^3}$ possible paths.

In this comparison, signs for each term have been assumed. However, verification of the signs of each term is required for correctness. For each box $D_{i,j}$, a unique sign assignment $\sigma^*$ is defined where it is either $1$ or $-1$. In order to test for the value of $\sigma^*$, it is factored in the comparison in Fredman's Trick. This increases the dimension of the space by $g^2$ due to the signs, and increases choices by $2^{g^2}$.
...
The most widely known and general distance metric is the Euclidean distance. For $m\times n$ matrices $A, B$ (which can be constructed by taking each vector in a time series as a row vector of the matrix), the norm of the difference between the matrices using the Frobenius norm is computed, which is a generalization of the Euclidean distance for vector spaces of matrices. This can be formulated as
\[
